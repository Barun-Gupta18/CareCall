...
import '../public/ServiceDescription.css';



function UserSubCategoryDetails() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  async function getSubcategoryData() {
    try {
      ...
      const url = Server_URL + "user-subcategory-details/" + id;
  ...
  async function getParticularPartner() {
    try {
      // console.log('dghj')
      const token = utilityFunctions.getCookieValue('userAuthToken');
      const url = Server_URL + "user-particular-partner/" + id;
      const response = await axios.get(url, {
        headers: {
          Authorization: token ? `Bearer ${token}` : ""
        }
  ...
  async function sendPartnerId(pid) {
    navigate('/user/partner-details', { state: { pid } });
  }
                ...
                <div className="col-lg-4 col-md-6" key={index}>
                  <div className="service-wrapper mb-30">
                    <div className="service-img">
                      <img src={`${Server_URL2
                        }${value.photo}`} alt={value.fullname}
...
export default UserSubCategoryDetails;
